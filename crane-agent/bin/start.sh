...
#kill old agent
id=`ps -ef | grep com.cip.crane.agent.StartServer | grep -v grep | awk '{print $2}'`
if [ "$id" != "" ]; then
...
nohup /usr/local/jdk/bin/java  -Djava.security.krb5.realm=DIANPING.COM -Djava.security.krb5.kdc=10.2.6.103:10.2.6.152 -classpath "conf/:lib/*"  com.cip.crane.agent.StartServer >/dev/null &
echo "start success"
#end
