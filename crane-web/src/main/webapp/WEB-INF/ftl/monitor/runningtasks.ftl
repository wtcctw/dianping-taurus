...
<#if dto.status == "RUNNING">
	<tr id="${dto.attemptID!}">
		...
		<td><#if mHelper.isViewLog(dto.execHost) == false><a taget="_blank" href="${rc.contextPath}/viewlog?id=${dto.attemptID!}&status=${dto.status}">日志</a><#else>Job机负载过高，无法查看实时日志</#if></td>
	</tr>
</#if>
</#list>
</#if>