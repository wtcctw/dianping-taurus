...
<#if dto.status == "TIMEOUT">
	...
	<#assign lastTaskStatus = mHelper.getLastTaskStatus(dto.taskID)>
		...
		<td><span class='label <#if lastTaskStatus == "SUCCEEDED" || lastTaskStatus == "RUNNING">label-info<#else>label-important</#if>'>${lastTaskStatus!}</span></td>
		<td><a taget="_blank" href="${rc.contextPath}/mvc/viewlog?id=${dto.attemptID!}&status=${dto.status}">日志</a></td>
		<td><a id='timeOutFeedBtn' class='feedBtn' href="${rc.contextPath}/mvc/feederror?id=${dto.attemptID!}&status=${dto.status!}&taskName=${task.name!}&ip=${dto.execHost!}&taskId=${dto.taskID!}&feedtype=wechat&from=monitor"><img border='0' src='${rc.contextPath}/img/wechat.png'  width='20' height='20' color='blue' alt='点我报错' title='点我报错'/></a> |<a id ='timeOutFeedBtn' class='feedBtn'  href='${rc.contextPath}/mvc/feederror?id=${dto.attemptID!}&status=${dto.status!}&taskName=${task.name!}&ip=${dto.execHost!}&taskId=${dto.taskID!}&feedtype=qq&from=monitor'><img border='0' src='${rc.contextPath}/img/qq.png'  width='20' height='20' color='blue' alt='点我报错' title='点我报错'/></a></td>
	<#break>
	</#if>
	</#list>
	</tr>
