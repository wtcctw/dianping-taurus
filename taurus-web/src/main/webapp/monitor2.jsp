...
<div class="sidebar " id="sidebar">
    ...
    <ul class="nav nav-list">
        ...
        <li id="monitor">
            <a href="monitor2.jsp" target="_self">
...
<div class="time_inal ">
    ...
    <div >
        ...
        <a class="atip"
           href="monitor2.jsp?step=<%=step%>&op=day&date=<%=df.format(new Date())%>"
        ...
        <a class="atip"
                <% if (now == null) {
                    ...
                    step = -720;
                %>
           href="monitor2.jsp?step=<%=step%>&op=day&date=<%=df.format(new Date(df.parse(now).getTime() + step*hourTime))%> "
           data-toggle="tooltip" data-placement="top"
    ...
    ~<%=formatter.format(new Date(df.parse(now).getTime()))%>]">[-1d] </a>
        ...
        <a class="atip"  <% if (now == null) {
           ...
           href="monitor2.jsp?step=-24&date＝<%=df.format(time)%> "
                <% } else {%>
           href="monitor2.jsp?step=<%=step%>&op=day&date=<%=df.format(new Date(df.parse(now).getTime() + step*hourTime))%> "
                <% }
                %>
        ...
        <a class="atip"  <% if (now == null) {
            ...
            step = 168;
            if (df.parse(now).after(time)) {%>
           href="monitor2.jsp?step=-24&op=day&date＝<%=df.format(time)%> "
                <% } else {%>
           href="monitor2.jsp?step=<%=step%>&op=day&date=<%=df.format(new Date(df.parse(now).getTime() + step*hourTime))%> "
                <% }
                %>
